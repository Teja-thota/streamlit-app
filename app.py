...
def live_camera_detection():
    ...
    while st.session_state.is_camera_active:
        ret, frame = cap.read()
        if not ret:
...
if 'is_camera_active' not in st.session_state:
    st.session_state.is_camera_active = False
...
if page == "Home":
    ...
    if start_button:
        st.session_state.is_camera_active = True  # Set camera to active state
        live_camera_detection()

    # Stop Button Logic
    stop_button = st.button("Stop", key="stop_live_detection")
    if stop_button:
        st.session_state.is_camera_active = False  # Set camera to inactive state
